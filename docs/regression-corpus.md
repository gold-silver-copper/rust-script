...
| Dependency check_parser assertion on control bytes | `fn\x03<{}` (control byte 0x03 mid-source) trips a `left == right` round-trip assertion at `ra_ap_syntax 0.0.342` `validation.rs:204` inside `check_parser`, found by a 2026-07-19 campaign; the product `parse_bytes` path rejects it cleanly without panic | `check_parser` is now called only on rustscript-admitted trees, not arbitrary bytes, since the dependency's own fuzz helper is not hardened against malformed input and this binary is `panic = abort`; regression input `fuzz/regressions/parser_bytes/check_parser_control_byte.txt` |
